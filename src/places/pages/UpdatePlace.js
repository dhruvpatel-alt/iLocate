...
const UpdatePlace = () => {
    ...
    const fetchPlace = async () => {
      try {
        const responseData = await sendRequest(
          `${process.env.REACT_APP_BACKEND_URLAAAAAPI}/places/${placeId}`
        );
  ...
  const placeUpdateSubmitHandler =async event => {
    ...
    try{
      await sendRequest(`${process.env.REACT_APP_BACKEND_URLAAAAAPI}/places/${placeId}`,"PATCH",JSON.stringify({
  ...
  return ( 
    ...
    {isLoading && (
      ...
      <h2 className="heading">Edit what you want </h2>
